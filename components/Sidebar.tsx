...
export default function Sidebar({ activeView, onSwitch, onToast }: SidebarProps) {
  ...
  const Item = ({ item }: { item: NavItem }) => {
    ...
    return (
      <div
        onClick={() => item.view ? onSwitch(item.view) : item.action?.()}
  ...
  return (
    <aside className="overflow-y-auto" style={{ background: "var(--white)", borderRight: "1px solid var(--gray-lt)" }}>
